...
export function ListProvider({ children }) {
  const [items, setItems] = useState(generateItemList(14));
  const [selectedItem, setSelectItems] = useState(null);

  return (
    <ListContext.Provider value={{ items, setSelectItems, selectedItem }}>
      {children}
